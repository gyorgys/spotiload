...
#ifdef _WIN32
#	if !defined(_WIN32_WINNT) && !defined(_WINDOWS_)
...
public:
	...
	bool trylock()	{ return TryEnterCriticalSection(&critsec) != 0; }
#endif
};
...
public:
    ...
    void lock()     { enter(); }
    void unlock()   { leave(); }
	bool trylock()	{ return pthread_mutex_trylock(&mtx); }
};
...
public:
    ...
    virtual ~scopelock()  { mtx->unlock(); }
};

#if (_WIN32_WINNT >= 0x0400) || !defined(_WIN32)
class scopetrylock: public noncopyable
{
protected:
	mutex* mtx;
	bool m_locked;
public:
	scopetrylock(mutex& imtx): mtx(&imtx), m_locked(false)  { m_locked = mtx->trylock(); }
	virtual ~scopetrylock()  { if( m_locked ) mtx->unlock(); }
